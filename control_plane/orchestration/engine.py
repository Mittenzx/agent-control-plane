...
from ..core.interfaces import Task, TaskStatus, Agent, AgentStatus
...
class WorkflowEngine:
    ...
    def on_task_completed(self, task: Task):
        ...
        if all(t.status in (TaskStatus.COMPLETED, TaskStatus.FAILED) for t in workflow.tasks):
            workflow.status = TaskStatus.COMPLETED if all(t.status == TaskStatus.COMPLETED for t in workflow.tasks) else TaskStatus.FAILED
