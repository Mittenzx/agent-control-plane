...
class AgentRegistry:
    ...
    def _notify_status_change(self, info: AgentInfo, old_status: AgentStatus, new_status: AgentStatus):
...
class AgentLifecycleManager:
    ...
    async def execute_task(self, agent: Agent, task: Task) -> Dict[str, Any]:
        ...
        if not agent.can_handle_task(task):
            raise ValueError(f"Agent {agent.name} cannot handle task requiring '{task.required_capability}'")
    ...
    async def shutdown_agent(self, agent: Agent, force: bool = False) -> None:
        ...
        if running_task and not force:
            raise ValueError(f"Agent {agent.name} has running task {running_task.id}. Use force=True to cancel.")
    ...
    async def shutdown_all(self, force: bool = False) -> None:
        ...
        await asyncio.gather(*[
            self.shutdown_agent(agent, force=force)
            for agent in agents
        ])