...
import asyncio
import logging
from typing import Dict, List, Optional, Callable, Any, Set, Awaitable, Union
...
# Handler may be sync or async (awaitable)
Handler = Union[Callable[[Message], None], Callable[[Message], Awaitable[Any]]]
...
class MessageBus:
    ...
    def __init__(self):
        ...
        self._broadcast_subscriptions: Set[Handler] = set()
    ...
    def unsubscribe(self, agent_id: str, handler: Handler) -> None:
    ...
    async def _deliver_direct(self, message: Message) -> None:
        ...
        queue = self._agent_addresses.get(message.recipient_id)  # type: ignore[arg-type]  # pre-existing: Optional recipient
        if queue:
            try:
        ...
        handlers = self._subscriptions.get(message.recipient_id, set())  # type: ignore[arg-type]  # pre-existing: Optional recipient
...
class RequestResponseBus(MessageBus):
    ...
    async def respond(
        ...
        future = self._pending_requests.pop(original_message.correlation_id, None)  # type: ignore[arg-type]  # pre-existing: Optional correlation
    ...
    def handle_response(self, message: Message) -> bool:
        ...
        future = self._pending_requests.pop(message.correlation_id, None)  # type: ignore[arg-type]  # pre-existing: Optional correlation
        if future and not future.done():
